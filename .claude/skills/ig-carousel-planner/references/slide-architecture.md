...
> **Local copy (TensPla):** API-limit statements, the em-dash rule and the hashtag-strategy pointer were removed. See `.claude/skills/PROVENANCE.md`.

Per-formula slide spines and the on-image text rules `ig-carousel-planner` uses.
Design for the portrait 4:5 frame, which fills the most screen.
...
CTA, and hold a few relevant hashtags.
